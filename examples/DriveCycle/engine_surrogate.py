...
def evaluate_example(surrogateToLoad="engine-surrogate"):
    ...
    evaluate_algorithm.add(
        ...
        mods.Variable(name="C3H6%20mass%20fraction%20%5B%2D%5D",
                      type="output"),
    ...
    export_cuds(evaluate_simulation, file=os.path.join(
        dir_path, "engine_in.ttl"), format="ttl")
    ...
    export_cuds(eva[0], file=os.path.join(
        dir_path, "engine_all.ttl"), format="ttl")

    export_cuds(output_data[0], file=os.path.join(
        dir_path, "engine_out.ttl"), format="ttl")
