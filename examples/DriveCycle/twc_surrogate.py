...
def evaluate_example(surrogateToLoad="twc-model-6"):
    ...
    evaluate_algorithm.add(
        ...
        mods.Variable(
            name="Inlet%20CO%20mass%20fraction%20%5B%2D%5D", type="input"),
        mods.Variable(
            name="Inlet%20CO2%20mass%20fraction%20%5B%2D%5D", type="input"),
        mods.Variable(
            name="Inlet%20C3H6%20mass%20fraction%20%5B%2D%5D", type="input"),
        mods.Variable(
            name="Inlet%20H2%20mass%20fraction%20%5B%2D%5D", type="input"),
        mods.Variable(
            name="Inlet%20H2O%20mass%20fraction%20%5B%2D%5D", type="input"),
        mods.Variable(
            name="Inlet%20NO%20mass%20fraction%20%5B%2D%5D", type="input"),
        mods.Variable(
            name="Inlet%20O2%20mass%20fraction%20%5B%2D%5D", type="input"),
        mods.Variable(
            name="Outlet%20CO%20mass%20fraction%20%5B%2D%5D", type="output"),
        mods.Variable(
            name="Outlet%20CO2%20mass%20fraction%20%5B%2D%5D", type="output"),
        mods.Variable(
            name="Outlet%20C3H6%20mass%20fraction%20%5B%2D%5D", type="output"),
        mods.Variable(
            name="Outlet%20H2%20mass%20fraction%20%5B%2D%5D", type="output"),
        mods.Variable(
            name="Outlet%20H2O%20mass%20fraction%20%5B%2D%5D", type="output"),
        mods.Variable(
            name="Outlet%20NO%20mass%20fraction%20%5B%2D%5D", type="output"),
        mods.Variable(
            name="Outlet%20O2%20mass%20fraction%20%5B%2D%5D", type="output")
    )
    ...
    example_data = [
        ...
        [0., 300., 9614., 0.00423872, 0.070268, 0.0205358,
            7.336e-5, 0.053835, 0.0012947, 0.07026804],
        [1800., 908., 10626., 0.003077, 0.04493, 0.013256,
            5.7013293e-5, 0.063, 0.001863, 0.0449314]
    ]
    ...
    export_cuds(evaluate_simulation, file=os.path.join(
        dir_path, "twc_in.ttl"), format="ttl")
    ...
    export_cuds(eva[0], file=os.path.join(
        dir_path, "twc_all.ttl"), format="ttl")

    export_cuds(output_data[0], file=os.path.join(
        dir_path, "twc_out.ttl"), format="ttl")
    ...
    for input_data_point in list_input_data_point:
        input_value = find_data_point_item_by_name(
            input_data_point, input_name)
        output_data_point = input_data_point.get(rel=mods.isSourceOf)[0]
        output_value = find_data_point_item_by_name(
            output_data_point, output_name)
        print("%s=%s,%s=%s" %
